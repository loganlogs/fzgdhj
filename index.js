...
let pyProcess;
const defaultImageUrl = 'https://cdn.discordapp.com/attachments/1218778860338806806/1218785956551725097/template.png?ex=6608eddf&is=65f678df&hm=3d83454a2023d6cb2b0e10b4d4901668da12fcb52336ec4b3727d0963e78764b&';
let userData = {};
...
client.once('ready', () => {
  ...
  if (logsChannel) {
    const embed = new MessageEmbed()
      ...
      .addField('Nombre total de personnes dans les serveurs', totalMembersCount.toString(), true)
      .setFooter('⭐ Lookup France');
  
  
    logsChannel.send({ embeds: [embed] });
  } else {
    console.error(`Le canal de logs avec l'ID ${config.logsstartchannel} est introuvable.`);
  }
...
if (command === 'scrap') {
  ...
  let totalPlayers = 0;
  if (args.length === 0) {
    message.channel.send('Veuillez spécifier "start", "stop" ou "info" après la commande.');
    return;
  }
  ...
  if (subCommand === 'start') {
    ...
    updateInterval = setInterval(() => {
      ...
      fs.readdir(dbDirectoryPath, (err, files) => {
        ...
        const guildLogo = message.guild.iconURL({ dynamic: true, format: 'png', size: 256 });
        const guildBanner = message.guild.bannerURL({ dynamic: true, format: 'png', size: 4096 });
        const scrapEmbed = new MessageEmbed()
          ...
          .addField('<:memeified_monster_blue:1218958077596926074>    Nombre de bases de données', numberOfDBFiles.toString(), true)
          .addField('<:blue_moon:1218958081677856768> Nombre de personnes dans la base de données', totalLines.toString(), true) // Correction ici
          .setFooter('⭐ Lookup France')
          .setTimestamp();
    ...
    fs.readdir(dbDirectoryPath, (err, files) => {
      ...
      const infoEmbed = new MessageEmbed()
        ...
        .addField('Total des joueurs dans la base de données', totalLines.toString())
        .setFooter('⭐ Lookup France')
        .setTimestamp();
    ...
    message.channel.send('Commande invalide. Veuillez spécifier "start", "stop" ou "info" après la commande.');
  }
}
...
if (!config.owner.includes(message.author.id)) {
    ...
    pyProcess.on('close', code => {
      ...
      fs.readFile('requete.txt', 'utf8', (err, data) => {
        if (err) {
          console.error(err);
          message.reply('Une erreur est survenue lors de la lecture de requete.txt.');
          return;
        }
  ...
  if (command === 'setpic') {
    ...
    client.user.setAvatar(imageLink)
        .then(() => {
            message.channel.send('La photo de profil a été mise à jour avec succès.');
        })
        .catch((error) => {
            console.error('Erreur lors de la mise à jour de la photo de profil :', error);
            message.channel.send('Une erreur est survenue lors de la mise à jour de la photo de profil.');
        });
}
...
if (command === 'setname') {
  if (!config.owner.includes(message.author.id)) {
    ...
    client.user.setUsername(args.join(' '))
        ...
        .catch((error) => {
            ...
            message.channel.send('Une erreur est survenue lors de la mise à jour du nom du bot.');
        });
}
  ...
  fs.readFile('serveur.txt', 'utf8', (err, data) => {
    if (err) {
      console.error(err);
      return message.reply("Une erreur est survenue lors de la lecture des serveurs.");
    }
...
if (message.channel.id !== config.lookupchannel) {
  ...
  const loadingEmbed = new MessageEmbed()
    ...
    .setDescription('Votre requête est en cours de traitement. Veuillez patienter...');
  ...
  if (!found) {
    ...
    const sendMP = async () => {
      ...
      const attachment = new MessageAttachment(Buffer.from(foundResults), 'results.txt');
      ...
      try {
        ...
        message.channel.send('Une erreur s\'est produite lors de l\'envoi des résultats en MP.');
      }
    };

    // Création du gestionnaire d'événements pour le bouton
    const filter = i => i.customId === 'send_mp_button' && i.user.id === message.author.id;
  ...
  if (command === 'help') {
    ...
    .setDescription(`Utilisez ${prefix} suivi du nom de la commande pour l'utiliser.`)
    .addField('helplookup', `Pour voir les commandes par rapport aux commandes lookup !`)
    .addField('helpowner', `Pour voir les commandes par rapport aux commandes owner !`)
...
message.channel.send({ embeds: [embed] });
  ...
  if (command === 'helplookup') {
    ...
    .setTitle('Liste des commandes disponibles')
    .setDescription(`Utilisez ${prefix} suivi du nom de la commande pour l'utiliser.`)
    .addField('scrap', `Pour démarrer ou arrêter le scraping des données. Exemple: \n\`${prefix}scrap start\` pour démarrer, \`${prefix}scrap stop\` pour arrêter.`)
    .addField('lookup', `Pour rechercher les identifiants d'une personne. Exemple: \n\`${prefix}lookup <ID Discord>\``)
    .addField('link', `Pour associer des informations à une personne. Exemple: \n\`${prefix}link <License> (sa license fivem)\``)
    .addField('user', `Pour afficher les informations d'une personne. Exemple: \n\`\`${prefix}user <ID Discord>\`\``)
    .addField('note', `Pour mettre une note sur une personne. Exemple: \n\`${prefix}note <ID Discord> <la note>\``)
    .setFooter('⭐ Lookup France')
    .setImage(defaultImageUrl);
...
if (command === 'helpcfx') {
  ...
  .setColor('#0099ff')
  .setTitle('Liste des commandes disponibles')
  .setDescription(`Utilisez ${prefix} suivi du nom de la commande puis du cfx id du serveur en question !`)
  .addField('cfxadd', `Pour ajouter un serveur au scrapper  Exemple: \n\`${prefix}cfxadd <CFXID>\``)
  .addField('cfxinfo', `Pour avoir les info d'un serveur  Exemple: \n\`${prefix}cfxinfo <CFXID>\``)
  .addField('cfxlist', `Pour avoir la list des serveurs qui vont etre scrap  Exemple: \n\`${prefix}cfxlist\``)
  .setFooter('⭐ Lookup France')
  .setImage(defaultImageUrl);
...
if (command === 'helpowner') {
  ...
  .setTitle('Liste des commandes disponibles')
  .setDescription(`Utilisez ${prefix} suivi du nom de la commande pour l'utiliser.`)
  .addField('setpic', `Pour changer la photo de profil du bot. Exemple: \n\`${prefix}setpic <lien vers l'image>\``)
  .addField('setname', `Pour changer le nom du bot. Exemple: \n\`${prefix}setname <nouveau nom>\``)
  .addField('owner', `Pour ajouter un nouvel owner au bot. Exemple: \n\`${prefix}owner @mention\``)
  .addField('unowner', `Pour retirer un owner du bot. Exemple: \n\`${prefix}unowner @mention\``)
  .addField('owners', `Affiche la liste de tous les owners du bot.`)
  .addField('wl', `Pour ajouter un nouvel wl au bot. Exemple: \n\`${prefix}wl @mention\``)
  .addField('unwl', `Pour retirer un wl du bot. Exemple: \n\`${prefix}unwl @mention\``)
...
message.channel.send({ embeds: [embed] });
  ...
  if (command === 'link') {
    ...
    if (!userId || !infoType || !infoValue) {
      message.channel.send(`Utilisation: ${prefix}link <ID Discord> <Identifier> <Identifiant>`);
      return;
    }
    if (!userData[userId]) {
      userData[userId] = [];
...
if (command === 'owner') {
  ...
  if (!mention) {
    message.channel.send("Vous devez mentionner un utilisateur à ajouter comme owner.");
    return;
  }
  ...
  fs.writeFile('config.json', JSON.stringify(config, null, 2), (err) => {
    if (err) {
      ...
      message.channel.send("Une erreur s'est produite lors de l'ajout du nouvel owner. Veuillez réessayer.");
    } else {
...
if (command === 'unowner') {
  ...
  const mention = message.mentions.users.first();
  if (!mention) {
    message.channel.send("Vous devez mentionner un utilisateur à retirer comme owner.");
    return;
  }
  ...
  fs.writeFile('config.json', JSON.stringify(config, null, 2), (err) => {
    if (err) {
      console.error('Erreur lors de l\'écriture dans le fichier config.json :', err);
      message.channel.send("Une erreur s'est produite lors du retrait de l'owner. Veuillez réessayer.");
    } else {
...
if (command === 'wl') {
  ...
  const mention = message.mentions.users.first();
  if (!mention) {
    message.channel.send("Vous devez mentionner un utilisateur à ajouter comme wl.");
    return;
  }
  ...
  fs.writeFile('config.json', JSON.stringify(config, null, 2), (err) => {
    if (err) {
      console.error('Erreur lors de l\'écriture dans le fichier config.json :', err);
      message.channel.send("Une erreur s'est produite lors de l'ajout du nouvel wl. Veuillez réessayer.");
    } else {
...
if (command === 'unwl') {
  ...
  const mention = message.mentions.users.first();
  if (!mention) {
    message.channel.send("Vous devez mentionner un utilisateur à retirer comme wl.");
    return;
  }
  ...
  fs.writeFile('config.json', JSON.stringify(config, null, 2), (err) => {
    if (err) {
      console.error('Erreur lors de l\'écriture dans le fichier config.json :', err);
      message.channel.send("Une erreur s'est produite lors du retrait de l'wl. Veuillez réessayer.");
    } else {
...
if (command === 'buyer') {
  ...
  if (!duration) {
    message.channel.send(`Vous devez spécifier la durée de l\'accès. Exemple : \`${prefix}buyer <mention> 30d\``);
    return;
  }
  ...
  if (match) {
    ...
    message.channel.send("Format de durée invalide. Utilisez le format 1d, 365d etc...");
    return;
  }
  ...
  message.channel.send(`L'acces à été donné a ${userIdToGrantAccess} pour ${days} jours.`);
}
  ...
  if (noteData[userId]) {
    noteData[userId].forEach(note => {
      notes.push(note);
       ...
       infoEmbed.addField('Date de création du compte', member.createdAt.toUTCString());
